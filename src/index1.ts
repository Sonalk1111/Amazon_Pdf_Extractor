...
extractTableData(path)
  .then(function(table_data: any) {
    // console.log(table_data);
    const x = table_data.pageTables.map((x: any) => x.tables)
    ...
    // })
      ...
      const splitString = data.map((x: any) => x.split('\n')).flat()
      const removeEmptyString = splitString.filter((x: any) => x != '')
      fs.writeFileSync('result.txt', `${removeEmptyString}`)
